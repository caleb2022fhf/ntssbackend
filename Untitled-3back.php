<?php
/**
 * nyanza_all_in_one.php
 *
 * Single-file PHP app that combines:
 *  - Authentication (register / login / logout / me) using sessions
 *  - Chat (store & fetch messages)
 *  - Calendar (CRUD events)
 *  - Serves three frontend pages: Login, Chat, Calendar
 *
 * Requirements:
 *  - PHP 8+ with PDO MySQL extension
 *  - Create a single database and run the SQL below once
 *
 * SQL (run once):
 *
...
if ($method === 'POST' && ($uri === '/login' || $uri === '/auth/login')) {
    ...
    try {
        ...
        $_SESSION['user'] = [
            ...
            'label' => $user['id_number'] // label used in chat; change as needed
        ];
...
/* GET /calendar -> calendar page (public) */
